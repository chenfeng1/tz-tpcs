...
public interface AreaDao extends CrudRepository<Area, String> {
     ...
     * 根据id来查找
     */
    @Query("select a from Area as a where a.parent.code =:code")
    List<Area> findByParentCode(@Param("code") String code);

    /**
     * 根据level来查
     */
    @Query("select a from Area as a where a.level =:level order by a.code")
    List<Area> findByLevel(@Param("level") int level);

    /**
     * 根据 code 查找 name
     */
    @Query("select name from Area as a where a.code=:code")
