...
<div class="container">
	...
	<form class="form-horizontal" role="form" action="${path }/students/save">
			...
			<div class="col-md-2">
				...
				<select class="form-control" id="degree" name="degree">
					<c:forEach items="${degree }" var="d">
						<option value="${d }"><fmt:message key='enum.degree.${d }'></fmt:message></option>
					</c:forEach>
				</select>
			</div>
		</div>
			...
			<div class="col-md-2">
				<select class="form-control" id="loadStatus" name="loanStatus">
					<c:forEach items="${loanStatus }" var="loan">
						<option value="${loan }"><fmt:message key='enum.loanStatus.${loan }'></fmt:message></option>
					</c:forEach>
				</select>
			</div>
			...
			<div class="col-md-2">
				<select class="form-control" id="source" name="source">
					<c:forEach items="${source }" var="s">
						<option value="${s }" ><fmt:message key="enum.source.${s }" /></option>
					</c:forEach>
				</select>
			</div>
			<label for="qq" class="col-md-1 control-label">QQ号</label>
			<div class="col-md-2">
				<input type="text" id="qq" class="form-control" placeholder="qq号码" name="qq" />
			</div>
		</div>
		...
		<div id="more" style="display: none;">
			...
			<div class="form-group">
				...
				<div class="col-md-3">
					<input type="text" id="identityCard" class="form-control"
						placeholder="身份证号码" name="identityCard" />
				</div>
			...
			<div class="form-group">
				...
				<div class="col-md-2">
					<input type="date" id="graduationDate" class="form-control" name="graduationDate2" />
				</div>
				<label class="col-md-1 control-label">是否需要毕设</label>
				<div class="col-md-2">
					<label class="radio-inline"> <input type="radio"
						name="needDesign" value="true" />是
					</label> <label class="radio-inline"> <input type="radio"
						name="needDesign" value="false" />否
					</label>
				</div>
				<label for="designDate" class="col-md-1 control-label">毕设日期</label>
				<div class="col-md-2">
					<input type="date" id="designDate" class="form-control" name="designDate2" />
				</div>
			</div>
			<div class="form-group">
			...
			<div class="form-group">
				...
				<div class="col-md-2">
					<label class="radio-inline"> <input type="radio"
						name="gender" value="MALE" checked />男
					</label> <label class="radio-inline"> <input type="radio"
				...
				<div class="col-md-2">
					<select id="status" class="form-control" name="status">
						<c:forEach items="${status }" var="s" varStatus="c" >
							<option value="${s }"
								<c:if test="${c.count==5 }">selected</c:if>
				...
				<div class="col-md-2">
					<select id="level" class="form-control" name="level">
						<c:forEach items="${level }" var="s" varStatus="v">
							<option value="${s }"
								<c:if test="${v.count==5 }" >selected</c:if>
